...
public class CountriesInContinent extends CountryCommandHandler {
    ...
    public PreparedStatement prepareStatement(Connection connection, Map<String, String> args) throws SQLException {
        PreparedStatement statement = connection.prepareStatement("SELECT Code, country.Name, Continent, Region, country.Population, city.Name as Capital FROM country JOIN city ON city.ID=country.Capital WHERE Continent = ? ORDER BY population DESC LIMIT ?; ");
